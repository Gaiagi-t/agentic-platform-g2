...
const QUADRANTS = [
  {
    ...
    label: "Quick Wins",
    action: "INIZIA DA QUI",
    impatto: "alto" as const,
    facilita: "facile" as const,
  ...
  {
    key: "investimenti" as const,
    label: "Investimenti Strategici",
    action: "PIANIFICA",
    impatto: "alto" as const,
    facilita: "difficile" as const,
  ...
  {
    key: "bassovalore" as const,
    label: "Basso Valore",
    action: "EVITA",
    impatto: "basso" as const,
    facilita: "facile" as const,
...
const newProcess = (id: string): Process => ({
  id,
  name: "",
  description: "",
  impatto: "alto",
...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    const s = getState();
    const initial = s.processes.length > 0
      ? s.processes
      : [newProcess("p1"), newProcess("p2"), newProcess("p3")];
    ...
    <div className="max-w-4xl mx-auto w-full px-4 py-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate">Descrivi 3 processi del tuo lavoro e posizionali sulla matrice</p>
        </div>
      </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {(["facile", "difficile"] as const).map((v) => (
                      <button
                        key={v}
                        onClick={() => update(p.id, "facilita", v)}
                        className={`px-3 py-1 text-xs rounded-full font-semibold border transition-colors ${
                          p.facilita === v
                            ? "bg-navy text-white border-navy"
                            : "bg-white text-slate border-slate-200 hover:border-navy"
      ...
      {filledProcesses.length > 0 && (
        <div className="mb-8">
          <h2 className="text-sm font-bold text-navy uppercase tracking-wide mb-3">
            Dove l&apos;AI può generare impatto nel tuo lavoro?
          </h2>
          <div className="relative">
            ...
            <div className="flex gap-0">
              ...
              <div className="flex items-center justify-center w-6 shrink-0">
                <span className="text-xs text-slate/70 -rotate-90 whitespace-nowrap font-medium" style={{ writingMode: "vertical-rl", transform: "rotate(180deg)" }}>
                  Impatto
                </span>
              </div>
              {/* Grid */}
              <div className="grid grid-cols-2 gap-2 flex-1">
                {QUADRANTS.map((q) => {
                  const inQ = filledProcesses.filter(
                    (p) => p.impatto === q.impatto && p.facilita === q.facilita
                  );
                  return (
                    <div
                      key={q.key}
                      className={`${q.bg} ${q.border} border-2 rounded-xl p-4 min-h-[140px] flex flex-col`}
                    >
                      <div className="mb-2">
                        <p className="font-bold text-sm text-navy">{q.label}</p>
                        <p className="text-xs text-slate">
                          Impatto: {q.impatto === "alto" ? "Alto" : "Basso"} · Facilità: {q.facilita === "facile" ? "Facile" : "Difficile"}
                        </p>
                        <span className="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 bg-white/70 rounded border border-slate-200 text-slate-600">
                          {q.action}
                        </span>
                      </div>
                      <div className="flex flex-col gap-1 mt-auto">
                        {inQ.map((p) => (
                          <button
                            ...
                            className={`flex items-center gap-2 px-2 py-1 rounded-lg border text-xs font-medium transition-all ${
                              selected === p.id
                                ? "bg-navy text-white border-navy"
                                : `${q.badge} border hover:opacity-80`
                            }`}
                          >
            ...
            {filledProcesses.length > 0 && !selected && (
              <p className="text-xs text-primary mt-2 text-center">
                Clicca su un processo nella matrice per selezionarlo
              </p>
            )}
          </div>
        </div>
